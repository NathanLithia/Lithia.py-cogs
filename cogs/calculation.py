...
class Calculation(commands.Cog):
    ...
    async def Format(self, ctx, TYPE: str = None, *, INPUT: str = None):
        if TYPE == "TABLE":
            ...
            await ctx.reply(f"{type(INPUT)}")
        else:
            await ctx.reply(f"the method '{TYPE}' is not available, see {self.client.prefix}Format types") 
    ...
    async def Encrypt(self, ctx, key: str = None, *, data: str = "Hello World"):
        ...
        for i in range(len(data)):
            ...
            enc.append(enc_c)
        await ctx.reply(base64.urlsafe_b64encode("".join(enc).encode()).decode("utf8"))
    ...
    async def Encode(self, ctx, TYPE: str = None, *, INPUT: str = None):
        ...
        if TYPE == "BASE85":
            await ctx.reply(str(base64.b85encode(bytes(INPUT, 'utf-8')))[2:-1])
        elif TYPE == "BASE64":
            await ctx.reply(str(base64.b64encode(bytes(INPUT, 'utf-8')))[2:-1])
        elif TYPE == "BASE32":
            await ctx.reply(str(base64.b32encode(bytes(INPUT, 'utf-8')))[2:-1])
        elif TYPE == "BASE16":
            await ctx.reply(str(base64.b16encode(bytes(INPUT, 'utf-8')))[2:-1])
        elif TYPE == "types":
            await ctx.reply(f"Encoding types: {ENCODE_TYPES}")
        else:
            await ctx.reply(f"the method '{TYPE}' is not available, see {self.client.prefix}Encode types") 
    ...
    async def Decode(self, ctx, TYPE: str = None, *, INPUT: str = None):
        ...
        if TYPE == "BASE85":
            await ctx.reply(str(base64.b85decode(bytes(INPUT, 'utf-8')))[2:-1])
        elif TYPE == "BASE64":
            await ctx.reply(str(base64.b64decode(bytes(INPUT, 'utf-8')))[2:-1])
        elif TYPE == "BASE32":
            await ctx.reply(str(base64.b32decode(bytes(INPUT, 'utf-8')))[2:-1])
        elif TYPE == "BASE16":
            await ctx.reply(str(base64.b16decode(bytes(INPUT, 'utf-8')))[2:-1])
        elif TYPE == "types":
            await ctx.reply(f"Decoding types: {DECODE_TYPES}")
        else:
            await ctx.reply(f"the method '{TYPE}' is not available, see {self.client.prefix}Decode types")
    ...
    async def Hash(self, ctx, TYPE: str = None, *, INPUT: str = None):
        ...
        if TYPE == "MD5":
            await ctx.reply(str(hashlib.md5(bytes(INPUT, 'utf-8')).hexdigest()))
        elif TYPE == "SHA256":
            await ctx.reply(str(hashlib.sha256(bytes(INPUT, 'utf-8')).hexdigest()))
        elif TYPE == "SHA1":
            await ctx.reply(str(hashlib.sha1(bytes(INPUT, 'utf-8')).hexdigest()))
        elif TYPE == "types":
            await ctx.reply(f"Hashing types: {HASH_TYPES}")
        else:
            await ctx.reply(f"the method '{TYPE}' is not available, see {self.client.prefix}Hash types")
    ...
    async def Calculate(self, ctx, *, message: str = None):
        ...
        if message == None:
            return await ctx.reply(f"{self.client.prefix}calc 13.411/4.267```diff\n- 13.411/4.267\n+ 3.14295758143895```")
        if any(x not in mathswhitelist for x in message):
            return await ctx.reply("Error. illegal expression.")
        else:
            try:
                calculation = str('```diff\n- '+str(message)+'\n+ '+str(eval(message))+'```')
            except OverflowError:
                return await ctx.reply("Error. overflow error.")
            except SyntaxError:
                return await ctx.reply("Error. syntax error.")
            except ZeroDivisionError:
                return await ctx.reply("Error. cannot divide by zero.")
            except TypeError:
                return await ctx.reply("Error, TypeError")
            if len(calculation) > 2000:
                return await ctx.reply("Error, calculation exceeds 2000 characters.")
            else:
                await ctx.reply(calculation)
